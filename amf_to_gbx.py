...
def GetTransListForHelperStripping(amf_nodes):
    ...
    node_id_translation_list = [0] * len(amf_nodes)
    node_leftover_id = []
    ...
    for i in range(len(amf_nodes)):
        ...
        if node.name.endswith(strip_these):
            if not ("wrist" in node.name):
                ...
                node_id_translation_list[i] = node_id_translation_list[node.parent_node]
            else:
                ...
                parent = amf_nodes[node.parent_node]
                node_id_translation_list[i] = node_id_translation_list[parent.parent_node]
        else:
            # Each node that isn't getting removed gets its own new id starting from 0.
            node_id_translation_list[i] = next_id
