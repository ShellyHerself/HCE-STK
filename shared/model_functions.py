...
def SortNodes(node_list):
    ...
    for i in range(len(node_list)):
        ...
        while parent > -1:
            assert depth < 128, "node_list given to SortNodes() has an infinite loop in it, or has way too many nodes."
    ...
    for level in levels:
        level, _ = fstruct.SortSteptreeByNames(level)
    ...
    for i in range(len(ids_to_names)):
        old_ids_to_new_ids[i] = names_to_new_ids[ids_to_names[i]]


    return new_nodes, old_ids_to_new_ids
